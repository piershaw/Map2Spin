...
double r_bias = 0, g_bias = 0, b_bias = 0;
double r_scale = 1.1, g_scale = 0.6, b_scale = 1.1; 
...
BYTE r = 0;
BYTE g = 0;
BYTE b = 0;                 // used to extract red, green, blue components of pixel
...
for (int cmd_index = 3; cmd_index < argc; cmd_index++)
    ...
    if (_strnicmp( argv[cmd_index], "-B", 2) == 0)
       ...
       cmd_B = 1;
        ...
        int cindex1 = 0;
		int jj = strlen(cstring1);
            // find comma delimeter
            for (cindex1=0; cindex1 < jj; cindex1++)
                if (cstring1[cindex1] == ',')
                   break;
...
//else
   //hydra_color_map = hydra_color_map1; i need to fix this

int ggg = strlen(bitmapinputfileroot);
...
for (int e=0; e < ggg; e++)
    if (tilemapinputfileroot[e]=='.') { tilemapinputfileroot[e] = 0; break; }
